import logging
import os
from logging.handlers import RotatingFileHandler

# --- Basic Setup ---
LOG_DIR = "logs"
if not os.path.exists(LOG_DIR):
    os.makedirs(LOG_DIR)

# --- Formatters ---
# Define how log messages will be formatted.
console_formatter = logging.Formatter(
    "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
file_formatter = logging.Formatter(
    "%(asctime)s - %(name)s - %(levelname)s - %(message)s [in %(pathname)s:%(lineno)d]"
)

# --- Handlers ---
# Handlers determine where the logs are sent (e.g., console, file).

# 1. Console Handler (for real-time feedback during development)
console_handler = logging.StreamHandler()
console_handler.setLevel(logging.INFO)
console_handler.setFormatter(console_formatter)

# 2. Error Log Handler (for critical errors)
# RotatingFileHandler ensures log files don't grow indefinitely.
error_handler = RotatingFileHandler(
    os.path.join(LOG_DIR, "errors.log"), maxBytes=5 * 1024 * 1024, backupCount=2  # 5 MB
)
error_handler.setLevel(logging.ERROR)
error_handler.setFormatter(file_formatter)

# 3. User Messages Log Handler (for tracking user interactions)
messages_handler = RotatingFileHandler(
    os.path.join(LOG_DIR, "messages.log"),
    maxBytes=5 * 1024 * 1024,  # 5 MB
    backupCount=2,
)
messages_handler.setLevel(logging.INFO)
messages_handler.setFormatter(file_formatter)

# 4. General Application Log Handler
app_handler = RotatingFileHandler(
    os.path.join(LOG_DIR, "app.log"), maxBytes=5 * 1024 * 1024, backupCount=2  # 5 MB
)
app_handler.setLevel(logging.DEBUG)  # Catches all levels of logs
app_handler.setFormatter(file_formatter)


# --- Loggers ---
# Loggers are the entry points for the logging system.


def setup_loggers():
    """Sets up and configures all the loggers for the application."""

    # Root logger configuration (catches application-wide errors)
    logging.basicConfig(
        level=logging.INFO, handlers=[console_handler, error_handler, app_handler]
    )

    # --- Specific Loggers ---

    # Logger for user interactions
    messages_logger = logging.getLogger("UserMessages")
    messages_logger.setLevel(logging.INFO)
    messages_logger.addHandler(messages_handler)
    messages_logger.propagate = (
        False  # Prevents logs from being passed to the root logger
    )

    # Suppress noisy logs from the HTTPX library used by python-telegram-bot
    logging.getLogger("httpx").setLevel(logging.WARNING)

    logging.info("Logging configuration loaded successfully.")
